...
照支付中台的套路：**推 main → CI 构镜像 → Harbor(<harbor-host>) → SSH 部署 .51 → 飞书通知**。
...
- 访问：公司 LAN `http://<deploy-host>:8800` 进管理后台。
- 密钥：`/opt/feishu-agent/config.yaml`（飞书凭证 + 初始管理员密码），**仅在 .51，不入库不进镜像**。

## .51 现状（已就绪，无需再装）
Docker 29.1.3 + compose v2；`daemon.json` 已含 `insecure-registries:[<harbor-host>]`；
WireGuard `wg0=<deploy-host-wg>`；Harbor <harbor-host> 可达；:8800 空闲。

## 一次性准备

### 1) Harbor 建项目 + 机器人账号
- Harbor(<harbor-host>) 新建 **Private 项目 `feishu-agent`**。
...
```
HARBOR_REGISTRY=<harbor-host>     HARBOR_PROJECT=feishu-agent
HARBOR_USER=robot$feishu-agent+ci    HARBOR_PASSWORD=<token>     (Masked)
DEPLOY_HOST=<deploy-host>    DEPLOY_DIR=/opt/feishu-agent
...
- **手动**（在 .51 部署目录）：
  ```bash
  REGISTRY=<harbor-host> PROJECT=feishu-agent TAG=<sha> \
...
## 验证
```bash
curl -s http://<deploy-host>:8800/healthz        # {"ok":true}
docker logs -f feishu-agent                        # 看 bot 长连接 connected
```
浏览器开 `http://<deploy-host>:8800` 登录管理后台。
